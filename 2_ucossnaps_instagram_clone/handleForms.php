...
if (isset($_POST['insertPhotoBtn'])) {
	...
	if (isset($_POST['action']) && $_POST['action'] === 'edit') {
		...
		$description = $_POST['description']; // Make sure this is passed in the form
		$photo_name = $_FILES['photo_name']['name'] ?? null;
	
		if ($photo_name) {
			// Handle file upload
			$tempFileName = $_FILES['photo_name']['tmp_name'];
			$folder = "images/" . $photo_name;
	
			if (move_uploaded_file($tempFileName, $folder)) {
				$editResponse = editPhoto($pdo, $photo_id, $photo_name, $description);
				echo $editResponse['message'];
			} else {
				echo "Failed to upload new photo.";
			}
		} else {
			// If no new photo uploaded, just update description
			$existingPhoto = getPhotoByID($pdo, $photo_id);
			$editResponse = editPhoto($pdo, $photo_id, $existingPhoto['photo_name'], $description);
			echo $editResponse['message'];
		}
	}
